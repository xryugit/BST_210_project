...
BPX <- BPX %>%
  mutate(
    ...
    MAP4 = (1 / 3) * BPXSY4 + (2 / 3) * BPXDI4,
    avg_MAP = rowMeans(cbind(MAP1, MAP2, MAP3, MAP4), na.rm = TRUE)
  )
...
numeric_vars <- select(dat_clean, age, BMI, avg_MAP, income, screen_time, sedentary_minutes, total_cholesterol, sleep_hours, depression_score, vitamin_d, vitamin_b12, alcohol)
...
dat_subset <- select(dat_temp, SEQN, SLD010H, SLQ050, SLQ060, RIDAGEYR, RIAGENDR, RIDRETH3, ALQ120Q, BMXBMI, avg_MAP, INDFMMPI, screen_time, PAD680, LBDTCSI, depression_score, depression_category, LBXVIDMS, LBDB12SI)
