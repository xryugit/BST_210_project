...
# Histogram of cotinuous variables
...
```
```{r}
hist(dat_cleaned$Family_income_to_poverty_ratio,main = "Family_income_to_poverty_ratio", xlab = "Family_income_to_poverty_ratio")
```
```{r}
par(mfrow = c(1,2))
hist(dat_cleaned$Na,main = "Na",xlab = "Na(mmol/L)")
hist(dat_cleaned$K,main = "K", xlab = "K(mmol/L)")
```
```{r}
par(mfrow = c(1,2))
hist(dat_cleaned$Ca,main = "Ca",xlab = "Ca(mmol/L)")
hist(dat_cleaned$Cl,main = "Cl", xlab = "Cl(mmol/L)")
```
```{r}
hist(dat_cleaned$sedentary_activity,main = "sedentary_activity",xlab = "sedentary_activity(mins)")
