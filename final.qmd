...
sec_Q3_2 <- dat_clean[,c("sleep_diagnosed","BMI","alcohol","gender","race_ethnicity","age","income","depression_score","depression_category","avg_MAP","total_cholesterol")]
sec_Q3_2$sleep_diagnosed <- ifelse(dat_clean$sleep_diagnosed == "2",0,1)
```
...
## Compare the percent of coefficient change between 2020 and 2013 model

```{r}
(final_model_sec1$coefficients - final_model_pri$coefficients) /final_model_pri$coefficients
```
```{r}
final_model_sec1$coefficients
final_model_pri$coefficients
```
