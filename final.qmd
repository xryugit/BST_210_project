...
sec_Q1 <- dat_clean[,c("sleep_trouble","BMI","alcohol","gender","race_ethnicity","age","income","depression_score","depression_category","avg_MAP","total_cholesterol")]
sec_Q1$sleep_trouble <- ifelse(dat_clean$sleep_trouble == "2",0,1)
```
...
```{r}
high_influence <-  as.numeric(rownames(influencePlot(mod3,col="red")))
sen_dat_pri <- pri_Q[-high_influence, ]
mod_sen <- update(mod3, data = sen_dat_pri)
summary(mod_sen)$aic
...
# Elastic Net
x <- as.matrix(sec_Q2[, c("LBDSPHSI", "LBXSNASI", "LBXSKSI", "LBXSCLSI", 
                            "LBDSCASI", "LBDSIRSI", 
                            "LBXSUA", "LBDSGLSI", "LBDTCSI", 
                            "dp_score", "dp_cate", 'BMI', 'gender','race_ethnicity','income', 'age')])
...
logis_phs = glm(sleep_trouble ~ LBDSPHSI, data = sec_Q2, family = "binomial")
summary(logis_phs) 

# Sleep disorders and sodium
logis_sod = glm(sleep_trouble ~ LBXSNASI, data = sec_Q2, family = "binomial")
summary(logis_sod) 


# Sleep disorders and potassium
logis_k = glm(sleep_trouble ~ LBXSKSI, data = sec_Q2, family = "binomial")
summary(logis_k) 


# Sleep hours and chloride
logis_cl = glm(sleep_trouble ~ LBXSCLSI, data = sec_Q2, family = "binomial")
summary(logis_cl)  #highly significant


# Sleep disorders and calcium
logis_ca = glm(sleep_trouble ~ LBDSCASI, data = sec_Q2, family = "binomial")
summary(logis_ca) 

# Sleep disorders and iodine
logis_iod = glm(sleep_trouble ~ URXUIO, data = sec_Q2, family = "binomial")
summary(logis_iod) 


# Sleep hours and iron
logis_fe = glm(sleep_trouble ~ LBDSIRSI, data = sec_Q2, family = "binomial")
summary(logis_fe) 
...
# Insulin
logis_ins = glm(sleep_trouble ~ LBDINSI, data = sec_Q2, family = "binomial")
summary(logis_ins) #highly significant

# Uric Acid
logis_ura = glm(sleep_trouble ~ LBXSUA, data = sec_Q2, family = "binomial")
summary(logis_ura) 

# Blood glucose
logis_glu = glm(sleep_trouble ~ LBDSGLSI, data = sec_Q2, family = "binomial")
summary(logis_glu) #highly significant

# Nitrate
logis_nit = glm(sleep_trouble ~ URXNO3, data = sec_Q2, family = "binomial")
summary(logis_nit)

# Total cholesterol
logis_tch = glm(sleep_trouble ~ LBDTCSI, data = sec_Q2, family = "binomial")
...
### predict 15-16 by full model
```{r}
full_model <- glm(sleep_trouble ~.,data = pri_Q, family = "binomial")
predprob <- predict(object = full_model, newdata = sec_Q3, type = "response")
predicted <- ifelse(predprob > 0.5, 1, 0)
predicted <- factor(predicted, levels = c(0, 1))
...
```{r}
missing_ind <- dat_raw_2020 %>%
  mutate(across(everything(), ~ ifelse(is.na(.), 1, 0), .names = "miss_{col}"))

summary(glm(miss_sleep_trouble ~ BMI, data = missing_ind, family = binomial))
```
